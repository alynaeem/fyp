from abc import ABC
from typing import List

from playwright.sync_api import Page

from crawler.constants.constant import RAW_PATH_CONSTANTS
from crawler.crawler_instance.local_interface_model.leak.leak_extractor_interface import leak_extractor_interface
from crawler.crawler_instance.local_shared_model.data_model.entity_model import entity_model
from crawler.crawler_instance.local_shared_model.data_model.leak_model import leak_model
from crawler.crawler_instance.local_shared_model.rule_model import RuleModel, FetchProxy, FetchConfig, ThreatType
from crawler.crawler_services.redis_manager.redis_controller import redis_controller
from crawler.crawler_services.redis_manager.redis_enums import REDIS_COMMANDS, CUSTOM_SCRIPT_REDIS_KEYS
from crawler.crawler_services.shared.helper_method import helper_method


class _b2ykcy2gcug4gnccm6hnrb5xapnresmyjjqgvhafaypppwgo4feixwyd(leak_extractor_interface, ABC):
    _instance = None

    def __init__(self, callback=None):

        self.callback = callback
        self._card_data = []
        self._entity_data = []
        self.soup = None
        self._initialized = None
        self._redis_instance = redis_controller()
        self._is_crawled = False

    def init_callback(self, callback=None):

        self.callback = callback

    def __new__(cls, callback=None):

        if cls._instance is None:
            cls._instance = super(_b2ykcy2gcug4gnccm6hnrb5xapnresmyjjqgvhafaypppwgo4feixwyd, cls).__new__(cls)
            cls._instance._initialized = False
        return cls._instance

    @property
    def is_crawled(self) -> bool:
        return self._is_crawled

    @property
    def seed_url(self) -> str:
        return "http://b2ykcy2gcug4gnccm6hnrb5xapnresmyjjqgvhafaypppwgo4feixwyd.onion/"

    @property
    def developer_signature(self) -> str:
        return "Usman Ali:qDMP"


    @property
    def base_url(self) -> str:
        return "http://b2ykcy2gcug4gnccm6hnrb5xapnresmyjjqgvhafaypppwgo4feixwyd.onion/"

    @property
    def rule_config(self) -> RuleModel:
        return RuleModel(m_fetch_proxy=FetchProxy.TOR, m_fetch_config=FetchConfig.PLAYRIGHT, m_threat_type= ThreatType.LEAK)

    @property
    def card_data(self) -> List[leak_model]:
        return self._card_data

    @property
    def entity_data(self) -> List[entity_model]:
        return self._entity_data

    def invoke_db(self, command: int, key: str, default_value, expiry: int = None):

        return self._redis_instance.invoke_trigger(command, [key + self.__class__.__name__, default_value, expiry])

    def contact_page(self) -> str:
        return "EFE1A6E5C8AF91FB1EA3A170823F5E69A85F866CF33A4370EC467474916941042E29C2EA4930"

    def append_leak_data(self, leak: leak_model, entity: entity_model):

        self._card_data.append(leak)
        self._entity_data.append(entity)
        if self.callback:
            if self.callback():
                self._card_data.clear()
                self._entity_data.clear()

    def parse_leak_data(self, page: Page):

        page.wait_for_selector('article.book-card', timeout=15000)

        article_elements = page.locator('article.book-card').all()

        hrefs = [elem.get_attribute('data-href') for elem in article_elements if elem.get_attribute('data-href')]

        for href in hrefs:

            try:
                page.goto(href, timeout=30000, wait_until='networkidle')
            except Exception as e:
                print(f"Failed to navigate to {href}: {str(e)}")
                continue

            title_elem = page.locator('h2').first
            title = title_elem.inner_text().strip() if title_elem.count() > 0 else ''

            p_elements = page.locator('p[style*="margin-top:1rem"]').all()
            country = p_elements[0].inner_text().strip() if len(p_elements) > 0 else ''

            website = p_elements[1].inner_text().strip() if len(p_elements) > 1 else ''

            data_elem = page.locator('p:has-text("DATA SIZE")').first
            data_size = ''
            description = ''
            if data_elem.count() > 0:
                data_text = data_elem.inner_text().split('\n')
                data_size = data_text[0].replace('DATA SIZE :', '').strip() if data_text else ''
                description = '\n'.join(line.strip() for line in data_text[1:] if line.strip()) if len(
                    data_text) > 1 else ''

            img_elems = page.locator('div.image-card img').all()
            img_srcs = [img.get_attribute('src') for img in img_elems if img.get_attribute('src')]

            views_elem = page.locator('div.muted').first
            views = views_elem.inner_text().strip().replace(',', '') if views_elem.count() > 0 else '0'

            m_content = (
                f"Title: {title}\n"
                f"Country: {country}\n"
                f"Website: {website}\n"
                f"Data Size: {data_size}\n"
                f"Description: {description}\n"
                f"Views: {views}\n"
                f"Image Sources: {', '.join(img_srcs) if img_srcs else 'None'}"
            )
            ref_html = helper_method.extract_refhtml(website, self.invoke_db, REDIS_COMMANDS, CUSTOM_SCRIPT_REDIS_KEYS, RAW_PATH_CONSTANTS, page)

            card_data = leak_model(
                m_ref_html=ref_html,
                m_title=title,
                m_url=href,
                m_base_url=self.base_url,
                m_content=m_content,
                m_network=helper_method.get_network_type(self.base_url),
                m_screenshot=helper_method.get_screenshot_base64(page,title, self.base_url),
                m_important_content=description,
                m_weblink=[website] if website else [],
                m_content_type=["leaks"],
                m_data_size=data_size,
                m_logo_or_images=img_srcs if img_srcs else []
            )

            entity_data = entity_model(
                m_team="Black Shrantac",
                m_country=[country],
                m_scrap_file=self.__class__.__name__, )

            self.append_leak_data(card_data, entity_data)


