...
class _imncrewwfkbjkhr2oylerfm5qtbzfphhmpcfag43xc2kfgvluqtlgoid(leak_extractor_interface, ABC):
    ...
    def parse_leak_data(self, page: Page):
        ...
        card_blocks = page.query_selector_all("div.block")

        for card in card_blocks:
            title_tag = card.query_selector("h3.yellow-text")
            title = title_tag.text_content().strip() if title_tag else ""

            desc_tag = card.query_selector("p")
            m_content = desc_tag.text_content().strip() if desc_tag else ""

            words = m_content.split()
            important_content = ' '.join(words[:500]) if len(words) > 500 else m_content

            download_link = ""
            spoiler_content = card.query_selector("div.spoiler-content")
            if spoiler_content:
                a_tag = spoiler_content.query_selector("a[href]")
                if a_tag:
                    download_link = a_tag.get_attribute("href")

            image_urls = []
            images_div = card.query_selector("div.images")
            if images_div:
                img_tags = images_div.query_selector_all("img[src]")
                for img in img_tags:
