...
async def get_current_user_optional(auth: HTTPAuthorizationCredentials | None = Depends(security_optional)):
    if auth is None:
        return None
    try:
...
async def startup_warm_map_stats():
    if not _env_enabled("WARM_MAP_CACHE_ON_STARTUP", False):
        log.info("Map stats cache warmup skipped. Set WARM_MAP_CACHE_ON_STARTUP=true to enable.")
        return
...
async def startup_warm_feed_cache():
    if not _env_enabled("WARM_FEED_CACHE_ON_STARTUP", False):
        log.info("Feed cache warmup skipped. Set WARM_FEED_CACHE_ON_STARTUP=true to enable.")
        return
...
async def trigger_smart_update(current_user: dict | None = Depends(get_current_user_optional)):
    ...
    triggered_by = (current_user or {}).get("username") or (current_user or {}).get("name") or "dashboard"
...
async def stop_intelligence_update(current_user: dict | None = Depends(get_current_user_optional)):
